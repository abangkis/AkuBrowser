# AkuBrowser

## Short description

Build a private AI feed from the X, LinkedIn, Facebook, and Instagram sources you enable.

## Description

AkuBrowser turns busy social feeds into a more relevant, private AI timeline.

You choose each source AkuBrowser may read. The setup page initially suggests
X, LinkedIn, Facebook, and Instagram, but no social-domain permission is granted until you
explicitly confirm the selection and approve Chrome's permission request.
AkuBrowser reads only rendered content on enabled sources.

Key features:

- bounded, read-only feed capture;
- a local personal timeline;
- duplicate and related-event analysis;
- preference learning from explicit user feedback;
- visible source evidence and original-post links;
- local retention and reset controls.

AkuBrowser requires a separately installed companion runtime on Windows or
macOS. Setup links only to fixed assets in the official AkuBrowser GitHub
release. The `v0.7.9` Windows and macOS installers are unsigned and may trigger
Windows Security, antivirus, or Gatekeeper warnings. Setup directs users to
verify the published SHA-256 and use macOS **Privacy & Security > Open Anyway**
when needed, never to disable Gatekeeper. Linux installation is not available
in 0.7.9. The runtime stores product data on the user's device. AI features may
submit selected content to OpenAI through the user's Codex App.

AkuBrowser does not request social passwords or cookies, sell data, use data for
advertising, or load remotely hosted JavaScript.

## Category

Productivity

## URLs

- Homepage: https://github.com/abangkis/AkuBrowser
- Support: https://github.com/abangkis/AkuBrowser/issues
- Privacy: https://github.com/abangkis/AkuBrowser/blob/main/PRIVACY.md
