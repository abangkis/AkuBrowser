...
Assert-True ($releaseManifest.version -eq "0.7.5") "AkuBrowser release version is unexpected."
...
Assert-True ($bridgePackage.akuRuntimeRevision -eq "source-adapters-v85") "AkuBridge development runtime revision is unexpected."
if ($DistributionOnly) {
...
Assert-True ($domain -match 'ApplicationVersion\s*=\s*"0\.7\.5"') "AkuSidecar version boundary is unexpected."
Assert-True ($releaseManifest.components.akuSidecar.version -eq "0.7.5") "AkuSidecar release manifest version is unexpected."
