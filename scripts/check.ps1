...
Assert-True ($bridgePackage.akuRuntimeRevision -eq "source-fidelity-v57") "AkuBridge runtime revision is unexpected."
...
Assert-True ($domain -match 'ApplicationVersion\s*=\s*"1\.0\.0-dev\.12"') "AkuSidecar version boundary is unexpected."
...
Assert-True ($supervised.health.expect.version -eq "1.0.0-dev.12") "AkuSupervisor expects the wrong AkuSidecar version."
