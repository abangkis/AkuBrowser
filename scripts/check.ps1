...
Assert-True ($releaseManifest.version -eq "0.7.0-preview.3") "AkuBrowser release version is unexpected."
...
Assert-True ($bridgePackage.akuRuntimeRevision -eq "source-adapters-v76") "AkuBridge development runtime revision is unexpected."
if ($DistributionOnly) {
...
if (-not $DistributionOnly) {
    ...
    Assert-True ($supervised.health.expect.version -eq "0.7.0-preview.3") "AkuSupervisor expects the wrong AkuSidecar version."
