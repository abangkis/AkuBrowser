...
| D-156 | Remove evidence identity from model authority. Present short ordered candidate aliases, exclude prior identity fields from comparison context, require exact item and assessment cardinality, and bind every returned pair to the original evidence key by deterministic Go position before validation or persistence | Implemented after repeated App Server evaluations invented opaque evidence keys and caused otherwise successful captures to stop at reasoning |
| D-157 | Restore Inbox as a compact operational run ledger rather than the former calibration workbench. Keep it read-only and bounded to recent sessions; expose per-source captured, evaluated, selected, and added counts plus capture rounds, snapshots, scrolls, model time, terminal stage, and failure message without exposing raw observations | Implemented in the Go Sidecar as the lightweight debugging seam for routine Check for updates |
| D-158 | Treat acquisition-planner follow-up as optional enrichment. If a follow-up capture fails after one valid observation was already accepted, record the failed Bridge command, resume reasoning from the initial bounded capture, and surface the fallback in Inbox instead of discarding the source's valid candidates | Implemented after live validation added two X items but discarded five valid LinkedIn candidates on a changed follow-up frontier |
