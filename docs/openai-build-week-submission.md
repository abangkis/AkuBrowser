...
The first run leads into calibration before the Timeline opens. While its bounded sample is being prepared, AkuBrowser explains the finite-feed model, the authority of direct feedback, and the current source capabilities instead of leaving the user with an unexplained wait. That clean first sample is added to the local event index without spending a model turn on cross-author comparison; the full semantic resolver begins with later checks. If a transient reasoning failure leaves no validated calibration entry, the captured trace remains inspectable and one explicit **Check for updates again** action continues the funnel. Later, direct feedback can promote, replace, demote, and suppress ordinary candidates. Material updates and contradictions remain protected, while one discovery lane preserves useful surprise. Active sources are ranked together rather than displayed as separate or rigidly alternating feeds. If nothing clears the newness, materiality, and evidence boundary, AkuBrowser says `0 additions` and stops.
