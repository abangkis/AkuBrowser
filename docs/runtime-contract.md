...
the Native Messaging Host reject a marker newer than the active runtime before
opening SQLite or launching the process. The typed
`data_version_incompatible` / `reset_data` outcome is user-visible as **Newer
data detected**. A downgrade-capable installer must archive the complete newer
data directory before activating the older runtime, create a fresh directory,
and retain a receipt. Ordinary uninstall preserves data; only the explicit
platform **Full reset** choice removes data and downgrade archives.
...
Full reset is backup-first and idle-only. The health endpoint reports database health but never exposes the absolute database path. Operational diagnosis belongs in the compact Update Inbox and component-native tests. Inbox source cards report Captured, Skipped unchanged, Evaluated, Selected, and Added stages, total source elapsed time, aggregate model time, and selection yield. Total includes browser and orchestration overhead that model time excludes. A completed run with zero captured candidates is surfaced as a capture-reliability warning; a run with captured candidates but zero evaluation is explained as continuity bypass when unchanged resurfaced items were skipped before reasoning. Inspect flow adds typed native-resurface and prior-knowledge-overlap badges. An unchanged fail-fast replay remains inspectable and retains its trusted native source link, but has no `Should have selected` action because it was deliberately not evaluated; an evaluated overlap uses `Select despite overlap`. `GET /api/inbox/runs/{runId}/trace` accepts one of `captured`, `skipped`, `evaluated`, `selected`, or `added` and returns at most 20 rows per request. `POST /api/inbox/runs/{runId}/re-evaluate` still reuses a failed run's durable observation without requesting a new capture.
