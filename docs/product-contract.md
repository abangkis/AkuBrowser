...
- A captured video may use its poster image as a bounded Timeline preview, but AkuBrowser does not imply native playback capability. Clicking the video poster or its play cue opens the trusted native post in a new tab so playback remains under the source platform. Video posters are excluded from the image zoom gallery; ordinary captured images keep the existing local viewer behavior.
