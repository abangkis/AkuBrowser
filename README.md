...
Its AI Detector adds a separate, explicitly uncertain layer for AI origin signals. A local deterministic pass can annotate retained text immediately, while an asynchronous Codex pass may confirm, dispute, or correct that preliminary assessment. These signals never affect selection or ranking. Inline is the default; users may instead route strong signals into a generic side pane or, after an exact typed warning, hide only direct or Deep-confirmed results. Direct user correction has the highest presentation authority.
