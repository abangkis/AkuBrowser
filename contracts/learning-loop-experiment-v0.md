...
Every promoted or reviewed item can switch independently between two presentations over the same captured evidence. `Brief` keeps AkuBrowser's normalized summary. `Source layout` reconstructs a source-inspired reading layout from captured candidate text, provenance, and bounded source-media URLs without opening or reading the source page again and without claiming to reproduce the live source DOM exactly. It does not add a generic `Captured evidence` banner inside every card; capture provenance remains a data/diagnostic boundary rather than duplicated reading chrome. LinkedIn Source layout preserves separate social context, author/headline/connection/time metadata, promotion or collaboration attribution, paragraph structure, and bounded job attachment presentation when those fields were captured. Source images are lazy-loaded without a referrer and may cause a request to the allowlisted source CDN when the browser cache cannot satisfy them. Source layout is the configurable default; the switch is item-local rather than a page-level tab.
...
The additive SQLite ledger, append-only preference events, provider invocation telemetry, explicit Codex configuration, bounded paged Review Inbox, newest-run expansion, symmetric Unified View signals, captured Source layout, and hard-gated Offline Preference Experiment v0 are implemented. The active preference vocabulary contains only `more_like_this` and `less_like_this`; retired development events are removed. Historical runs remain readable but are not falsely backfilled as evaluated candidates because their exact provider prompt boundary cannot be reconstructed. Preference profile v0 intentionally remains in `collecting` state; shadow fitting is blocked and learned ranking weights, exploration, and comeback behavior remain inactive until natural candidate labels pass every gate.
